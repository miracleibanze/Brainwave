...
const Header = () => {
  ...
  return (
    <div className={`fixed top-0 w-screen z-15 bg-n-8/90 lg:bg-n-8/90 flex items-center lg:px-8 md:px-6 px-4 ${openNavigation ? 'bg-n-8' : 'bg-n-8/90 backdrop-blur-sm'}`}>
        <div className="flex items-center px-2 lg:px-7.5 xl:px-10 max-lg:py-4"></div>

        <a>
        <img src={brainwave} alt="Brainwave" width={180} height={50} className='my-4 lg:w-[12rem] md:w-[10rem] w-[8rem]'/>
        </a>
        <nav className={`${openNavigation ? 'flex' : 'hidden' } fixed top-[5rem] left-0 right-0 bottom-0 bg-n-8 lg:static lg:flex lg:mx-auto lg:bg-transparent`}>
            ...
            <Link to="features" smooth={true} className='m-0 p-0'><a href="#feature" onClick={handleClick}>Features</a></Link>
